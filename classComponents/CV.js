...
class CV extends Component {
  ...
  render() {
    ...
    function deleteSection(e) {
      ...
      const sectionTypeIsEducation = parentSection.parentElement.classList.value.includes('education');
      const sectionTypeIsWork = !sectionTypeIsEducation;
      if (sectionTypeIsEducation) {
        if (educationSectionElemsArray.length < 2) return;
        parentSection.classList.add('hidden');
      }
      if (sectionTypeIsWork) {
        if (workExperienceSectionElemsArray.length < 2) return;
        parentSection.classList.add('hidden');
      }
    }  
